...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/EstilosArtistaAdmin.css') }}">
...
<body>
    ...
    <div class="container mt-5">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <form action="{{route('artistasAdmin.update', $art->id)}}" method="POST" onsubmit="return validateForm();" class="p-4 bg-light rounded shadow">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="name">Nombre</label>
                        <input type="text" name="name" id="name" class="form-control" value="{{$art->nombre}}" maxlength="255">
                    </div>
                    <div class="form-group">
                        <label for="description">Descripción</label>
                        <textarea name="description" id="description" cols="30" rows="10" class="form-control" maxlength="255">{{$art->descripcion}}</textarea>
                        <div id="description-counter" class="text-muted mt-2"></div>
                    </div>
                    <button type="submit" class="btn btn-primary mt-4">Guardar modificacion</button>
                </form>
            </div>
        </div>
    </div>
